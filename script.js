...
// ========== FIXED: STATIC GALLERY WITH AUTOMATIC SLIDESHOW ==========
...
function initializeSlideshow() {
    ...
    if (slides.length === 0) {
        console.error('No slides found in the gallery');
        return;
    }
...
function showPrevImage() {
    ...
    const img = slides[currentImageIndex].querySelector('img');
    if (img && modalImage) {
        modalImage.src = img.src;
    }
}
...
function openNewCertificateModal(index) {
    if (!newCertificateModal || !newCertificateContent) return;
